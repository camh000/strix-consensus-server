...
async def handle_consensus_mode(request: ChatRequest, config: Dict):
    ...
    if use_judge and len(worker_responses) > 1:
        ...
        # Log the decision
        await log_consensus_decision(
            request, worker_responses, best_response, reasoning
        )
        ...
        if not worker_responses:
            logger.error("No worker responses available")
            raise HTTPException(
                status_code=503,
                detail="No workers available - models may still be loading",
            )
    ...
    try:
        ...
        if config.judge and config.judge.get("model"):
            if config_manager.config.get("judge"):
                ...
                logger.info(
                    f"Updated judge model from {old_model} to: {config.judge['model']}"
                )
