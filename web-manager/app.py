...
def api_status():
    ...
    try:
        ...
        return jsonify(
            {"error": str(e), "mode": "unknown", "workers": {}, "stats": {}}
        ), 503
...
def set_mode():
    ...
    try:
        response = requests.post(f"{ORCHESTRATOR_URL}/api/mode", json=data, timeout=5)
        return jsonify(response.json())
    except Exception as e:
...
def get_logs():
    ...
    try:
        ...
        response = requests.get(f"{ORCHESTRATOR_URL}/api/logs?limit={limit}", timeout=5)
        return jsonify(response.json())
    except Exception as e:
...
def list_models():
    """List available models from local models directory"""
    models = []
    ...
    try:
        if os.path.exists(models_dir):
            ...
            for filename in sorted(os.listdir(models_dir)):
                if filename.endswith(".gguf"):
                    ...
                    model_name = filename[:-5]  # Remove .gguf
                    ...
                    models.append(
                        {
                            ...
                            "filename": filename,
                            "description": f"Local model file",
                        }
                    )
        ...
        if not models:
            return jsonify(
                {
                    "models": [],
                    "message": "No models found. Add .gguf files to the ./models/ directory.",
                }
            )
...
def upload_model():
    ...
    return jsonify({"status": "success", "filename": file.filename, "path": filepath})
...
def update_model_config():
    ...
    try:
        ...
        response = requests.post(
            f"{ORCHESTRATOR_URL}/api/config/models", json=data, timeout=10
        )
...
def reload_models():
    ...
    try:
        # Forward to orchestrator
        response = requests.post(f"{ORCHESTRATOR_URL}/api/models/reload", timeout=5)
        return jsonify(response.json()), response.status_code
    except Exception as e:
...
downloads_store = {"active": [], "completed": [], "failed": []}
...
def shutdown():
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "message": "All services are shutting down..."}
        )
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_downloads():
    ...
    print(
        f"[API] /api/downloads called. Active: {len(downloads_store['active'])}, Completed: {len(downloads_store['completed'])}"
    )
    ...
    def download_worker():
        ...
        try:
            ...
            urls_to_try = [
                ...
                f"https://huggingface.co/{model_id}/resolve/main/{parts[-1]}-Q4_K_M.gguf"
                if len(parts) >= 2
                else "",
                f"https://huggingface.co/{model_id}/resolve/main/{parts[-1]}-Q5_K_M.gguf"
                if len(parts) >= 2
                else "",
            ]
            ...
            for url in urls_to_try:
                ...
                try:
                    ...
                    print(
                        f"[DOWNLOAD WORKER] Download completed successfully from: {url}"
                    )
            ...
            if not download_success:
                raise Exception(
                    f"Could not download model. Tried {len(urls_to_try)} URLs. Last error: {last_error}"
                )
            ...
            # Move to completed
            downloads_store["active"] = [
                d for d in downloads_store["active"] if d["id"] != download_id
            ]
            downloads_store["completed"].insert(0, download_entry)
            print(
                f"[DOWNLOAD WORKER] Moved to completed. Total completed: {len(downloads_store['completed'])}"
            )
            ...
            print(
                f"[DOWNLOAD WORKER] Moved to failed. Total failed: {len(downloads_store['failed'])}"
            )
